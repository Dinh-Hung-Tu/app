...
Orignal post http://norvig.com/spell-correct.html
Adapted to Python 3 
"""

import re, collections

# This function returns the function listing the set of words in the text documents, ignoring the numbers. 
def words(text): return re.findall('[a-z]+', text.lower()) # The + sign means match characters following by characters 
# words in fact. Otherwise the following function returns characters 
# def words2(text): return re.findall('[a-z]', text.lower()) 

# Counting the frequency of the words in the features space
def train(features):
    model = collections.defaultdict(lambda: 1) # Default value of this dictionary is 1
    for f in features: # like for each word in the space
        model[f] += 1 # increase the count
    return model #return a dictionary, in specific, implementation of default dict in the collection 

NWORDS = train(words(open('big.txt',encoding= "ISO-8859-1").read()))

alphabet = 'abcdefghijklmnopqrstuvwxyz'

# listing all the edit distance of 1 edit 
def edits1(word): 
   splits     = [(word[:i], word[i:]) for i in range(len(word) + 1)] # splits is the list of tuples ('somethi', 'ng')
   deletes    = [a + b[1:] for a, b in splits if b] # starting to miss one character b[0] "smething"
   transposes = [a + b[1] + b[0] + b[2:] for a, b in splits if len(b)>1] # swap the first two character in the b part "smoething"
   replaces   = [a + c + b[1:] for a, b in splits for c in alphabet if b] # replace one character by one the other 26 options
   inserts    = [a + c + b     for a, b in splits for c in alphabet] # insert character c in between a and b parts
   return set(deletes + transposes + replaces + inserts)

# listing all the edit distance of 2 edit (repeat the process above), with the known words
def known_edits2(word): 
    return set(e2 for e1 in edits1(word) for e2 in edits1(e1) if e2 in NWORDS) # Limit number of possible e2 to the set of known words

# Input is the list of words, not the single word/string
def known(words): return set(w for w in words if w in NWORDS)

def correct(word):
    candidates = known([word]) or known(edits1(word)) or known_edits2(word) or [word] # Finding all possible variation
    # including a correct term, the correct term would have highest frequency 
    # an extension could be showing two most likely outcomes. 
